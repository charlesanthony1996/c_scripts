...
// For the vectorized stores, view C and Bf as arrays of 4 floats.
...
// Blocking parameters (for NEON: 4 floats per vector)
...
// Matrix multiplication routine: C[sy:ey, :] = A[sy:ey, :] * B[:,:]
...
#ifndef __APPLE__
    // Set thread affinity if available (Linux, etc.)
    cpu_set_t set;
    CPU_ZERO(&set);
    CPU_SET(k, &set);
...
int main() {
...
#ifdef DEBUG
    // In debug mode, fill A and B with test values.
    for (int i = 0; i < N*N; i++) {
        A[i] = i;
        B[i] = i;
    }
    ...
    // Rearrange B into groups of 4 rows for efficient NEON loads.
    for (int y = 0; y < N; y += 4) {
        for (int x = 0; x < N; x++) {
    ...
    // Run the multiplication several times.
    for (int i = 0; i < 10; i++) {
...
#ifndef DEBUG
    // Validate computed C against the reference matrix.
    for (int k = 0; k < N * N; k++) {
